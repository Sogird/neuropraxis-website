  ...
  return (
    <main>
      <PageHeader
        ...
        description="Termine nach Vereinbarung. Bitte rufen Sie uns während unserer Telefonzeiten an oder vereinbaren Sie einen Rückruf."
      />
            ...
            <div>
              ...
              <div className="divide-y divide-border-subtle border border-border-subtle rounded-lg overflow-hidden">
                {sprechzeiten.map((s) => (
                  <div
                    key={s.tag}
                    className="flex items-center justify-between px-6 py-4 bg-bg-primary"
                  >
                    <span className="text-sm font-medium text-text-primary">{s.tag}</span>
                    <span className="text-sm text-text-secondary">{s.zeiten}</span>
                  </div>
                ))}
                <div className="flex items-center justify-between px-6 py-4 bg-bg-secondary">
                  <span className="text-sm text-text-muted">und nach Vereinbarung</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="divide-y divide-border-subtle border border-border-subtle rounded-lg overflow-hidden">
                {telefon.map((t) => (
                  <div
                    key={t.tage}
                    className="flex items-center justify-between px-6 py-4 bg-bg-primary flex-wrap gap-2"
                  >
                    <span className="text-sm text-text-secondary">{t.tage}</span>
                    <span className="text-sm font-medium text-text-primary">{t.zeiten}</span>
                  </div>
                ))}
              </div>
            </div>
